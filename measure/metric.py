...
class Fmeasure_and_FNR(object):
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray):
        pred, gt = _prepare_data(pred, gt)  # pred, GT全部压缩到0~1
...
class DICE(object):
    ...
    def cal_dice(self, pred: np.ndarray, gt: np.ndarray):
        ...
        "网上的写法"
        # intersection = pred * gt  # 计算交集  pred ∩ true
        # temp = pred + gt  # pred + true
        # smooth = 1  # 防止分母为 0
        # dice = (2 * intersection.sum() + smooth) / (temp.sum() + smooth)
        "写法3-polyp-pvt-soft label"
        smooth = 1
